...
class Node:
    ...
    def __init__(self):
        ...
        self.baud_rate_front = int(rospy.get_param("~baud_front", "115200"))
        self.address_front = self.get_address("~address", "128")
        self.dev_name_rear = rospy.get_param("~dev_rear", "/dev/ttyACM1")
        self.baud_rate_rear = int(rospy.get_param("~baud_rear", "115200"))
        self.address_rear = self.get_address("~address", "129")
        self.roboclaw_front = self.setup_devices(self.dev_name_front, self.baud_rate_front, self.address_front)
        self.roboclaw_rear = self.setup_devices(self.dev_name_rear, self.baud_rate_rear, self.address_rear)
        self.read_and_reset_all_devices()
        self.parameters_setup()
        self.diagnostic_status_setup()
        self.diagnotics_updater_setup()
    ...
    def diagnostic_status_setup(self):
    ...
    def diagnotics_updater_setup(self):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            status_front_rigth, enc_front_rigth, crc_front_rigth = None, None, None
            status_front_left, enc_front_left, crc_front_left = None, None, None
            status_rear_rigth, enc_rear_rigth, crc_rear_rigth = None, None, None
            status_rear_left, enc_rear_left, crc_rear_left = None, None, None

            try:
                status_front_rigth, enc_front_rigth, crc_front_rigth = roboclaw.ReadEncM1(self.addresses[0])
            except ValueError:
                pass
            except OSError as e:
            ...
            if (status_front_rigth is not None and status_front_left is not None and status_rear_rigth is not None and status_rear_left is not None):
                rospy.logdebug("Encoders %d %d %d %d" % (enc_front_rigth, enc_front_left, enc_rear_rigth, enc_rear_left))
                if (self.encodm):
                    self.encodm.update_publish(enc_front_rigth, enc_front_left, enc_rear_rigth, enc_rear_left)
                self.updater.update()
            self.movement.run()
    ...
    def check_vitals(self, stat):
        for address in self.addresses:
            ...
            try:
                stat.add("Main Batt V:", float(roboclaw.ReadMainBatteryVoltage(address)[1] / 10))
                stat.add("Logic Batt V:", float(roboclaw.ReadLogicBatteryVoltage(address)[1] / 10))
                stat.add("Temp1 C:", float(roboclaw.ReadTemp(address)[1] / 10))
                stat.add("Temp2 C:", float(roboclaw.ReadTemp2(address)[1] / 10))
                _, cur1, cur2 = roboclaw.ReadCurrents(address)
