    ...
    def update_publish(self, enc_front_left, enc_front_right, enc_rear_left, enc_rear_right):
        ...
        for encoder_name, reading in encoders.items():
            ...
            if abs(reading - last_reading) > self.ENCODER_JUMP_THRESHOLD:
                rospy.logerr(
                    "Ignoring " + encoder_name + " encoder jump: cur " + reading + ", last "
                    + last_reading
                )
                return
        ...
        vel_x, vel_theta = self.update(
            enc_front_left, enc_front_right, enc_rear_left, enc_rear_right
        )
...
class Node:
    ...
    def __init__(self):
        ...
        self.dev_names = [
            rospy.get_param("~dev_front", "/dev/rcFront"),
            rospy.get_param("~dev_rear", "/dev/rcBack"),
        ]
        self.addresses = [
            self.get_address("~address_front", "128"),
            self.get_address("~address_rear", "129"),
        ]
        self.baud_rate = int(rospy.get_param("~baud", "460800"))
        self.drivers = self.setup_devices(
            self.dev_names, self.addresses, self.baud_rate
        )
    ...
    def setup_devices(self, dev_names, addresses, baud_rate):
        ...
        for dev_name, address in zip(dev_names, addresses):
            try:
                ...
                rospy.logfatal(
                    f"Could not connect to Roboclaw at {dev_name} with address {str(address)}"
                )
                rospy.logdebug(e)
            ...
            if version is not None:
                if not version[0]:
                    rospy.logwarn(
                        f"Could not get version from Roboclaw at address: {str(address)}"
                    )
                else:
    ...
    def publishers_and_subscribers_setup(self):
        ...
        self.current_pub = rospy.Publisher(
            rospy.get_param("~name") + "/currents", Float32MultiArray, queue_size=10
        )
